...
parameters {
  ...
  row_vector[N_cov] beta;           // coefficients
...
model {
   ...
   // LIKELIHOOD
   for(n in 1:N)
     y[n] ~ normal(alpha + beta * x[n] + tau * z[n], sigma_t*z[n] + sigma_c*(1-z[n]));
}
...
generated quantities{
  ...
  { // to create temporary variables
    ...
    for(n in 1:N){
      ...
      real mu_t = alpha + beta*x[n] + tau;      // mean for the treated
      real mu_c = alpha + beta*x[n];            // mean for the control
